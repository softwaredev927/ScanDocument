...
- (void)createGLKView
{
    ...
    cSF = self.contentScaleFactor * 3;
    view.contentScaleFactor = cSF; //self.contentScaleFactor;
...
-(void)captureOutput:(AVCaptureOutput *)captureOutput didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection
{
    ...
    if (self.context && _coreImageContext)
    {
        ...
        CGRect targetRect = CGRectMake(0, 0, self.rt.size.width * cSF, self.rt.size.height * cSF);
        CGRect newRect = [self getAspectFitSize:[image extent] targetRect:targetRect];
        [_coreImageContext drawImage:image inRect:newRect fromRect:[image extent]];
...
- (CGRect )getAspectFitSize:(CGRect ) originRect targetRect:(CGRect )targetRect {
    CGFloat ratioX = originRect.size.width / targetRect.size.width;
    CGFloat ratioY = originRect.size.height / targetRect.size.height;
    CGFloat ratio = (ratioX < ratioY) ? ratioX : ratioY;
    CGFloat newWidth = originRect.size.width / ratio;
    CGFloat newHeight = originRect.size.height / ratio;
    CGFloat newX = CGRectGetMidX(targetRect) - newWidth / 2;
    CGFloat newY = CGRectGetMidY(targetRect) - newHeight / 2;
    return CGRectMake(newX, newY, newWidth, newHeight);
    
    
//    CGFloat ratio = originRect.size.width / originRect.size.height;
...
//        newHeight = targetRect.size.width / ratio;
//        newWidth = targetRect.size.width;
//    }else{
//        newWidth = targetRect.size.height * ratio;
//        newHeight = targetRect.size.height;
//    }
//
//    newX = CGRectGetMidX(targetRect) - newWidth / 2;
//    newY = CGRectGetMidY(targetRect) - newHeight / 2;
//    return CGRectMake(newX, newY, newWidth, newHeight);
}
...
{
    CIImage *overlay = [CIImage imageWithColor:[CIColor colorWithRed:0  green:0.602  blue:0.732 alpha:0.5]];
...
- (CIDetector *)rectangleDetetor
{
    ...
    {
          detector = [CIDetector detectorOfType:CIDetectorTypeRectangle context:nil options:@{CIDetectorAccuracy : CIDetectorAccuracyLow, CIDetectorTracking : @(NO)}];
    });
    return detector;
}
...
- (CIDetector *)highAccuracyRectangleDetector
{
    ...
    dispatch_once(&onceToken, ^
    {
        detector = [CIDetector detectorOfType:CIDetectorTypeRectangle context:nil options:@{CIDetectorAccuracy : CIDetectorAccuracyLow}];
    });
    return detector;
}
